...
class LighterPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    @property
    def name(self) -> str:
        # Note: domain here refers to the entire exchange name. i.e. lighter_perpetual or lighter_perpetual_testnet
        return self._domain
    ...
    def trading_rules_request_path(self) -> str:
        return CONSTANTS.ORDER_BOOK_DETAILS_URL
    ...
    async def _update_trading_rules(self):
        async def fetch_order_book_detail(market_id: int):
            try:
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg="Could not fetch user events from Lighter Perpetual. Check API key and network connection.",
                )
    ...
    def _format_trading_rules(self, raw_details: List[Dict[str, Any]]) -> List[TradingRule]:
        return_val: list = []

        for ticker in self.market_tickers:
            details = next((d for d in raw_details if d and d.get("market_id") == ticker["market_id"]), None)
            if not details:
                continue
            try:
                trading_pair = ticker["trading_pair"]
                step_size = Decimal(f"1e-{details['supported_size_decimals']}")
                price_size = Decimal(f"1e-{details['supported_price_decimals']}")
                min_order_size = Decimal(details["min_base_amount"])
                ...
                self.logger().error(f"Error parsing trading rule for {ticker['trading_pair']}", exc_info=True)
    ...
    async def _update_balances(self):
        ...
        total_balance = Decimal(first_account["total_asset_value"])
        available_to_withdraw = Decimal(first_account["collateral"])
    ...
    async def _trading_pair_position_mode_set(self, mode: PositionMode, trading_pair: str) -> Tuple[bool, str]:
        ...
        if initial_mode != mode:
            msg = "lighter_perpetual only supports the ONEWAY position mode."
            success = False
        return success, msg
    ...
    def _last_funding_time(self) -> int:
        """
        Funding settlement occurs every 1 hours as mentioned in https://lighter-perpetual.gitbook.io/lighter-perpetual-docs/trading/funding
        """
